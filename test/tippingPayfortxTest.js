...
describe('Tipping Contract', () => {
  ...
  it('Tipping Contract: Reclaim Tip', async () => {
    await contract.methods.tip('reclaimtip.test', '1', {amount: 1});
    await contract.methods.tip('reclaimtip.test', '2', {amount: 2});

    const state = TippingContractUtil.getTipsRetips((await contract.methods.get_state()).decodedResult);
    const tip = state.tips.find(tip => tip.url === 'reclaimtip.test' && tip.title === '1');
    assert.equal(state.urls.find(url => url.url === 'reclaimtip.test').unclaimed_amount, 3);
    ...
    assert.equal(stateAfter.urls.find(url => url.url === 'reclaimtip.test').unclaimed_amount, 6);
  });
  ...
  it('Tipping Contract: Reclaim Tip Claimed', async () => {
    await contract.methods.tip('reclaimtip.test', '3', {amount: 8});

    const checkClaim = await contract.methods.check_claim('reclaimtip.test', wallets[1].publicKey);
    assert.equal(checkClaim.result.returnType, 'ok');
    assert.equal(checkClaim.decodedResult.success, true);
    assert.equal(checkClaim.decodedResult.percentage, 80);

    const state = TippingContractUtil.getTipsRetips((await contract.methods.get_state()).decodedResult);
    const tip = state.tips.find(tip => tip.url === 'reclaimtip.test' && tip.title === '3');

    assert.equal(state.urls.find(url => url.url === 'reclaimtip.test').unclaimed_amount, 8 + 2 + 4);

    const claim = await contract.methods.claim('reclaimtip.test', wallets[1].publicKey, false);
    assert.equal(claim.result.returnType, 'ok');

    const stateAfter = TippingContractUtil.getTipsRetips((await contract.methods.get_state()).decodedResult);
    assert.equal(stateAfter.urls.find(url => url.url === 'reclaimtip.test').unclaimed_amount, 0);
  ...
  it('Tipping Contract: Reclaim ReTip', async () => {
    await contract.methods.tip('reclaimretip.test', '1', {amount: 1});
    const state = TippingContractUtil.getTipsRetips((await contract.methods.get_state()).decodedResult);
    const tip = state.tips.find(tip => tip.url === 'reclaimretip.test' && tip.title === '1');
    await contract.methods.retip(tip.id, {amount: 2});

    const stateRetip = TippingContractUtil.getTipsRetips((await contract.methods.get_state()).decodedResult);
    const retip = stateRetip.tips.find(tip => tip.url === 'reclaimretip.test' && tip.title === '1').retips[0];

    const balanceBefore = await client.balance(contract.deployInfo.address.replace('ct_', 'ak_'));
    const reclaim = await contract.methods.reclaim_retip(retip.id);
    assert.equal(reclaim.result.returnType, 'ok');
    const balanceAfter = await client.balance(contract.deployInfo.address.replace('ct_', 'ak_'));
    assert.equal(parseInt(balanceAfter) + 2, parseInt(balanceBefore));

    const stateAfter = TippingContractUtil.getTipsRetips((await contract.methods.get_state()).decodedResult);
    assert.equal(stateAfter.urls.find(url => url.url === 'reclaimretip.test').unclaimed_amount, 1);
  });
